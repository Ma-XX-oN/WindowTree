...
using hwnd_to_owned_ptr_t = my_map<HWND, node_t*>;
...
struct node_t
{
	...
	node_t* m_pOwner = nullptr;
	bool parent_owner_both_null = false;

	// HWND to child nodes (node_t*)
	hwnd_to_children_ptr_t m_hwnd_to_child_node;

	// HWND to owned nodes (node_t*)
	hwnd_to_owned_ptr_t m_hwnd_to_owned_node;
	...
	// Adds owned node to this node.
	void add_owned(node_t * pOwned);

	// Outputs nodes and children
	void output_node_and_decendents(std::wostream& os, int indent = 0);
